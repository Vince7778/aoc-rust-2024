...
fn dfs(s: usize, d: usize, memo: &mut HashMap<(usize, usize), usize>) -> usize {
    ...
    let ans = if s == 0 {
        ...
        let m: usize = 10_usize.pow((digits as u32)/2);
...
pub fn part_one(input: &str) -> Option<u32> {
    ...
    Some(i.into_iter().map(|x| dfs(x as usize, 25, &mut memo)).sum::<usize>() as u32)
}

pub fn part_two(input: &str) -> Option<usize> {
    let i = ints(input);
    let mut memo = HashMap::new();
    Some(i.into_iter().map(|x| dfs(x as usize, 75, &mut memo)).sum::<usize>() as usize)
}

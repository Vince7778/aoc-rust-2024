...
pub fn part_one(input: &str) -> Option<usize> {
    ...
    let mut dv: HashMap<PosDir, isize> = HashMap::new();
...
pub fn part_two(input: &str) -> Option<usize> {
    ...
    let mut pq: BinaryHeap<(isize, (usize, usize), usize)> = BinaryHeap::new();
    let mut prevs: HashMap<PosDir, Vec<PosDir>> = HashMap::new();
    pq.push((0, s, 1));
    while let Some((dist, p, d)) = pq.pop() {
        let dist = -dist;
        if p == e {
    ...
    let mut vis: HashSet<PosDir> = HashSet::new();
    let mut stk: Vec<PosDir> = Vec::new();
    for i in 0..4 {
        stk.push((e, i));
        vis.insert((e, i));
    }
